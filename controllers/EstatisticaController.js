...
exports.obterEstatisticasGerais = async (req, res) => {
  try {
    const estatisticas = await Falha.findAll({
      attributes: [
        [sequelize.fn('COUNT', sequelize.col('id')), 'totalFalhas'],
        'status',
        [sequelize.fn('DATE', sequelize.col('dataCriacao')), 'data']
      ],
      group: ['status', 'data']
    });
